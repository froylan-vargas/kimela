...
describe('CreateQimelaUseCase', () => {
  ...
  describe('phase validation', () => {
    ...
    it('throws UnprocessableEntityException when initialPhase is ACTIVE', async () => {
      // Arrange
      mockPrisma.phase.findUnique
        .mockResolvedValueOnce(makePhase(INITIAL_PHASE_ID, 1, EVENT_ID, 'ACTIVE'))
        .mockResolvedValueOnce(makePhase(FINAL_PHASE_ID, 2, EVENT_ID, 'UPCOMING'));
    ...
    it('throws UnprocessableEntityException when finalPhase is ACTIVE', async () => {
      // Arrange
      mockPrisma.phase.findUnique
        .mockResolvedValueOnce(makePhase(INITIAL_PHASE_ID, 1, EVENT_ID, 'UPCOMING'))
        .mockResolvedValueOnce(makePhase(FINAL_PHASE_ID, 2, EVENT_ID, 'ACTIVE'));
  ...
  describe('successful creation', () => {
    ...
    it('creates a qimela with UPCOMING status when initialPhase is not ACTIVE', async () => {
      ...
      mockPrisma.phase.findUnique
        .mockResolvedValueOnce(makePhase(INITIAL_PHASE_ID, 1, EVENT_ID, 'UPCOMING'))
        .mockResolvedValueOnce(makePhase(FINAL_PHASE_ID, 2, EVENT_ID, 'UPCOMING'));
      mockQimelaRepository.save.mockResolvedValue(makeSavedEntity(QimelaStatus.UPCOMING));
