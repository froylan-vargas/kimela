...
export class GenerateInviteTokenUseCase {
  constructor(
    @InjectPinoLogger(GenerateInviteTokenUseCase.name)
  ...
  async execute(
    command: GenerateInviteTokenCommand,
    ...
    if (!qimela.isCreatedBy(command.requesterId)) {
      throw new ForbiddenException(
        "Solo el creador puede generar un enlace de invitación",
      );
    }

    const existing = await this.inviteTokenRepository.findByQimelaId(
      command.qimelaId,
    );
