...
const mockLogger: any = { trace: jest.fn(), debug: jest.fn(), info: jest.fn(), warn: jest.fn(), error: jest.fn(), fatal: jest.fn() };
  ...
  describe('execute', () => {
    it('throws NotFoundException when token not found', async () => {
      ...
      // Act + Assert
      await expect(useCase.execute(VALID_TOKEN)).rejects.toThrow(NotFoundException);
    });

    it('throws GoneException when token is revoked', async () => {
      // Arrange
      mockInviteTokenRepository.findByToken.mockResolvedValue(makeInviteToken({ revoked: true }));
    ...
    it('returns qimela public info on success', async () => {
      // Arrange
      mockInviteTokenRepository.findByToken.mockResolvedValue(makeInviteToken());
