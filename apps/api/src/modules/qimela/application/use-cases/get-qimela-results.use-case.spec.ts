...
const mockLogger: any = { trace: jest.fn(), debug: jest.fn(), info: jest.fn(), warn: jest.fn(), error: jest.fn(), fatal: jest.fn() };
  ...
  it('returns comparison data with real result, picks and points per user', async () => {
    ...
    mockPrisma.subscription.findFirst.mockResolvedValue({ id: 'sub-id' });
    mockPrisma.phase.findFirst.mockResolvedValue({ id: PHASE_ID });
    mockPrisma.subscription.findMany.mockResolvedValue([{ userId: OTHER_USER_ID }]);
    ...
    mockPrisma.session.findMany.mockResolvedValue([
      {
        ...
        contenders: [
          { role: 'home', contender: { id: 'home-id', name: 'Italia', imgUrl: null } },
          { role: 'away', contender: { id: 'away-id', name: 'México', imgUrl: null } },
        ],
        results: [
    ...
    mockPrisma.userPick.findMany.mockResolvedValue([
      { userId: USER_ID, sessionId: 'session-1', pickCategoryId: 'cat-home', value: '2' },
      { userId: USER_ID, sessionId: 'session-1', pickCategoryId: 'cat-away', value: '1' },
      { userId: OTHER_USER_ID, sessionId: 'session-1', pickCategoryId: 'cat-home', value: '1' },
    ]);
  ...
  it('returns sessions without official result and leaves points empty until scoring exists', async () => {
    ...
    mockPrisma.session.findMany.mockResolvedValue([
      {
        ...
        phase: { id: PHASE_ID, name: 'Jornada 1' },
        contenders: [
          { role: 'home', contender: { id: 'home-id', name: 'Brasil', imgUrl: null } },
          { role: 'away', contender: { id: 'away-id', name: 'Argentina', imgUrl: null } },
        ],
        results: [],
      },
    ]);
    ...
    mockPrisma.userPick.findMany.mockResolvedValue([
      { userId: USER_ID, sessionId: 'session-2', pickCategoryId: 'cat-home', value: '1' },
      { userId: USER_ID, sessionId: 'session-2', pickCategoryId: 'cat-away', value: '1' },
      { userId: OTHER_USER_ID, sessionId: 'session-2', pickCategoryId: 'cat-home', value: '2' },
      { userId: OTHER_USER_ID, sessionId: 'session-2', pickCategoryId: 'cat-away', value: '0' },
    ]);
    mockPrisma.userSessionPoints.findMany.mockResolvedValue([]);
    ...
    expect(result.data[0]).toEqual(
      expect.objectContaining({
        ...
        users: [
          expect.objectContaining({ userId: USER_ID, homePick: '1', awayPick: '1', points: null }),
          expect.objectContaining({ userId: OTHER_USER_ID, homePick: '2', awayPick: '0', points: null }),
        ],
      }),
    );
  });
  ...
  it('returns null points for users without picks', async () => {
    ...
    expect(result.data[0].users).toEqual([
      expect.objectContaining({ userId: USER_ID, homePick: null, awayPick: null, points: null }),
      expect.objectContaining({ userId: OTHER_USER_ID, homePick: null, awayPick: null, points: null }),
    ]);
  });
});
