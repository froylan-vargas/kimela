...
export class GetSessionTop5PicksUseCase {
  ...
  constructor(
    @InjectPinoLogger(GetSessionTop5PicksUseCase.name) private readonly logger: PinoLogger,
  ...
  async execute(
    ...
    this.logger.info(`Getting top5 picks for session ${sessionId} in qimela ${qimelaId}`);
    ...
    const qimela = await this.prisma.qimela.findUnique({
      ...
      select: {
        ...
        subscriptions: { select: { user: { select: { id: true, name: true } } } },
      },
    });
    ...
    const scoreCategories = await this.prisma.pickCategory.findMany({
      where: { sportId: qimela.sportId, name: { in: ['score_home', 'score_away'] } },
      select: { id: true, name: true },
    });
    const homeCatId = scoreCategories.find((c) => c.name === 'score_home')?.id ?? null;
    const awayCatId = scoreCategories.find((c) => c.name === 'score_away')?.id ?? null;

    const participants = this.buildParticipants(qimela);
    const overall = await this.buildTop5(qimelaId, participants, sessionId, homeCatId, awayCatId);

    let phase: Top5PickEntry[] | null = null;
    if (phaseId) {
      phase = await this.buildTop5ForPhase(qimelaId, phaseId, participants, sessionId, homeCatId, awayCatId);
    }
  ...
  private async buildTop5ForPhase(
    ...
    const aggregated = new Map<string, { totalPoints: number; exactResultsCount: number }>();
  ...
  private async attachPicks(
    ...
    const picks = categoryIds.length > 0
      ? await this.prisma.userPick.findMany({
          where: { userId: { in: userIds }, sessionId, pickCategoryId: { in: categoryIds } },
          select: { userId: true, pickCategoryId: true, value: true },
        })
      : [];

    const pickKey = (uid: string, catId: string) => `${uid}::${catId}`;
    const pickMap = new Map(picks.map((p) => [pickKey(p.userId, p.pickCategoryId), p.value]));

    return ranked.map((entry) => ({
      rank: entry.rank,
      userId: entry.userId,
      userName: entry.userName,
      homePick: homeCatId ? pickMap.get(pickKey(entry.userId, homeCatId)) ?? null : null,
      awayPick: awayCatId ? pickMap.get(pickKey(entry.userId, awayCatId)) ?? null : null,
    }));
  }
  ...
  private buildParticipants(qimela: {
    ...
    map.set(qimela.creator.id, { userId: qimela.creator.id, userName: qimela.creator.name });
  ...
  private async assertAccess(userId: string, qimelaId: string, creatorId: string): Promise<void> {
