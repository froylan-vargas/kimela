...
describe("QimelaController", () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: GetSportsForQimelaUseCase,
          useValue: { execute: jest.fn() },
        },
        {
          provide: GetEventsForQimelaUseCase,
          useValue: { execute: jest.fn() },
        },
        ...
        {
          provide: GetUpcomingSessionsUseCase,
          useValue: { execute: jest.fn() },
        },
        ...
        {
          provide: GetQimelaSubscribersUseCase,
          useValue: { execute: jest.fn() },
        },
        {
          provide: RemoveSubscriptionUseCase,
          useValue: { execute: jest.fn() },
        },
        ...
        {
          provide: getLoggerToken(QimelaController.name),
          useValue: {
            trace: jest.fn(),
            debug: jest.fn(),
            info: jest.fn(),
  ...
  describe("getById", () => {
    it("calls use case with the provided id and user id", async () => {
      ...
      // Assert
      expect(getQimelaById.execute).toHaveBeenCalledWith(
        QIMELA_ID,
        MOCK_USER_ID,
      );
    });
  ...
  describe("getSessionTop5", () => {
    it("delegates to use case with qimelaId, sessionId, userId, and phaseId", async () => {
      ...
      const result = await controller.getSessionTop5(
        QIMELA_ID,
        "session-1",
        MOCK_USER,
        "phase-1",
  ...
  describe("savePicks", () => {
    it("delegates to use case with qimelaId, sessionId, userId and picks", async () => {
      ...
      expect(saveSessionPicks.execute).toHaveBeenCalledWith({
        ...
        userId: MOCK_USER_ID,
        picks: [
          { pickCategoryId: "cat-1", value: "2", pickedContenderId: null },
        ],
      });
    });
  });
        ...
        data: [
          {
            ...
            name: "Soccer",
            imgUrl: null,
            sessionFormat: "MATCHUP" as const,
          },
        ],
      };
  ...
  describe("update", () => {
    ...
    it("returns updated qimela from use case", async () => {
      ...
      // Act
      const result = await controller.update(QIMELA_ID, MOCK_USER, {
        name: "Updated Name",
      });
  ...
  describe("createLabel", () => {
    it("calls createQimelaLabel.execute with qimelaId, requesterId, name, and color", async () => {
      // Arrange
      const mockResponse = {
        data: { id: LABEL_ID, name: "VIP", color: "#ff0000" },
      };
      createQimelaLabel.execute.mockResolvedValue(mockResponse);

      // Act
      await controller.createLabel(QIMELA_ID, MOCK_USER, {
        name: "VIP",
        color: "#ff0000",
      });
    ...
    it("returns created label from use case", async () => {
      ...
      // Act
      const result = await controller.createLabel(QIMELA_ID, MOCK_USER, {
        name: "VIP",
        color: "#ff0000",
      });
  ...
  describe("applyLabelToSubscriber", () => {
    it("calls applyLabel.execute with qimelaId, requesterId, targetUserId, and labelId", async () => {
      ...
      // Act
      await controller.applyLabelToSubscriber(
        QIMELA_ID,
        TARGET_USER_ID,
        LABEL_ID,
        MOCK_USER,
    ...
    it("returns { data: { applied: true } }", async () => {
      ...
      // Act
      const result = await controller.applyLabelToSubscriber(
        QIMELA_ID,
        TARGET_USER_ID,
        LABEL_ID,
        MOCK_USER,
  ...
  describe("removeLabelFromSubscriber", () => {
    it("calls removeLabel.execute with qimelaId, requesterId, targetUserId, and labelId", async () => {
      ...
      // Act
      await controller.removeLabelFromSubscriber(
        QIMELA_ID,
        TARGET_USER_ID,
        LABEL_ID,
        MOCK_USER,
    ...
    it("returns { data: { removed: true } }", async () => {
      ...
      // Act
      const result = await controller.removeLabelFromSubscriber(
        QIMELA_ID,
        TARGET_USER_ID,
        LABEL_ID,
        MOCK_USER,
